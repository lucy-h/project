...
User
	...
	last_name:string
	email:string
	building:string // building where the student lives
...
A wishlist is a list of items from a single store that users want to by. The list also has a "want by" date that states the latest day that a user would want to buy the things on this wishlist. Essentially, the "want by" date is an expiration date for the wishlist.

3) Store
	name:string
	url:string
	shipping_minimum:float // minimum amount spent to get free shipping, or to minimize shipping costs
	shipping_cost:float // shipping cost for an order costing at least the shipping minimum (usually $0, but sometimes a small fee)
...
4) Item
	...
	url:string
	price:float
An item in a store. An item belongs to a single store. The URL links to the item on the store's website.
